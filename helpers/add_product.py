...
def add_product(session, product, brand, menu, datetime):
    ...
    if product_in_db:
        ...
        brand_in_db = session.query(brand).filter(
            brand.brand_name.ilike(product_brand)).one_or_none()
        if brand_in_db:
            product.brand_id = brand_in_db.brand_id
        else:
            brand = brand(brand_name=product_brand)
            session.add(brand)
            session.commit()
